...
# 2. Generate Bradley-Terry pairwise data

generate_bt_data <- function(w_true, N) {
  ...
  for (k in 1:ncol(item_pairs)) {
    ...
    p_i_wins <- w_true[item_i] /
      (w_true[item_i] + w_true[item_j])
...
run_bt_simulation <- function(w_true, N, B) {
  ...
  for (b in 1:B) {
    ...
    pairwise_matrix <- generate_bt_data(w_true, N)
    
    thurstone_order <- fit_thurstone(pairwise_matrix)$item
    bt_order <- fit_bt(pairwise_matrix)$item
    ...
    simulation_results$thurstone_distance[b] <-
      kendall_distance(thurstone_order, true_order)
    
    simulation_results$bt_distance[b] <-
      kendall_distance(bt_order, true_order)
  }
...
# 7. Run all Bradley-Terry-generated settings

set.seed(123)

bt_simulation_summary <- data.frame()

for (score_name in names(score_list)) {
  
  for (N in sample_sizes) {
    
    results <- run_bt_simulation(
    ...
    bt_simulation_summary <- rbind(
      bt_simulation_summary,
      setting_summary
    )
  }
}
